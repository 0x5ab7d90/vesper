/* eslint-disable @typescript-eslint/explicit-function-return-type -- vendored, kept close to upstream */
/*
 * ikousikdas · OpenShaders — https://openshaders.com/@ikousikdas
 * WebGPU · dither
 *
 * Vendored from apps/site (hero-shader.webgpu.ts); keep the two in step. The upstream React
 * wrapper was dropped; shader-fill.tsx and the site's hero-backdrop.tsx drive
...
export type ShaderOptions = {
  ...
  maxPixels?: number
  /** Added to the vendored source: multiplies the clock, for surfaces that show only a
   *  sliver of the field and need its drift to read. */
  timeScale?: number
}
...
function animate(
  ...
  function tick(now: number) {
    ...
    previous = now
    if (autoplay) {
      if (!stillness.matches) elapsed += delta * (options.timeScale ?? 1)
